...
// обертка на pgxpool.Query
func (db *DB) Query(ctx context.Context, q Query, args ...interface{}) (pgx.Rows, error) {
	if tx, ok := ctx.Value(key.KeyTx).(pgx.Tx); ok {
...
// обертка на pgxpool.QueryRow
func (db *DB) QueryRow(ctx context.Context, q Query, args ...interface{}) pgx.Row {
	if tx, ok := ctx.Value(key.KeyTx).(pgx.Tx); ok {
